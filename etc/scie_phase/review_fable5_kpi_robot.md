...
>
> **상태: 발견 9건 전건 미수정.** 수정 시 각 항목의 `수정` 칸에 커밋 해시를 기입할 것.
...
명시적으로 거부 — 어느 쪽이든 가드와 FSM이 일치해야 함. | 수정: ______
...
정식 편입. 동결 필드명 유지 여부는 HANDOFF §3.7 규약 확인 후 결정. | 수정: ______
...
주문을 unserved 집계에 포함. _ops_span 경계 처리는 정의 변경이므로 HANDOFF §3.7과 정합 확인. | 수정: ______
...
산출 CSV를 csv.reader로 파싱해 전 행 3열 확인. | 수정: ______
...
**수정 방향**: 주석을 실제 구간 차이를 명시하는 서술로 교체. 논문 인용 시 주의 문구 포함. | 수정: ______
...
보고서 문자열에 0.735/0.738이 구분되어 나타나는지 확인. | 수정: ______
...
기존 스위트 + verify_hr green이 동작 불변의 증거. | 수정: ______

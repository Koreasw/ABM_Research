...
- 본 연구는 4가지 핸드오프 모드(H0–H3)를 **하나의 ABM** 에서 비교 평가하고, 수요 강도 K ∈ {50, 100}, 라이더 시간단가 w_R, locker 수 M 의 sweep 하에서 **3-stakeholder Pareto frontier** 를 도출한다. 빌딩 규모는 **5F × 500 m²/층 (Korean 소형 사무 표준) 으로 baseline 단일 고정** 하며, 10F 확장 (E3 typology) 은 1차 실험 결과 후 별도 진행.
- 참고 선행: 세종시 공공자전거 *어울링* data-driven ABM — bottom-up calibration 사례.

### 1.2 핵심 framing
배달의민족 데이터의 K=50 시나리오(약 1시간 horizon, 50건 주문)를 **한국 표준 소형 사무 빌딩 (5F × 500 m²/층 ≈ 150평/층, footprint 22m × 23m, ~280인 점유) 의 점심피크 1시간** 실측 수요 자료로 직접 해석한다. K=50 / horizon ≈ 1 h ⇒ 50 ord/h ⇒ 50/280 ≈ **18% 의 점심 주문률**, 한국 사무직의 점심 배달 문화에 자연 정합. 다운스케일 보정 ρ 없이 데이터 단위와 운영 단위가 직접 매핑된다. **본 paper 의 1차 실험은 K∈{50, 100} 만 다루며**, K=100 (36% 주문률) 은 동일 빌딩의 *수요 강도 stress 조건* (예: 금요일·악천후 시 배달 surge) 으로 해석. K∈{200, 300}, 10F E3 typology, K=500/1000 (tier-2) 은 5F-K=50/100 결과를 본 뒤 결정 (Future Work).
...
3. 라이더·고객·빌딩 3 주체의 KPI 를 동일 시뮬레이션 내에서 산출하고, **3차원 Pareto frontier** 를 K∈{50, 100} 의 1차 실험 cohort 에서 추적 (확장 typology 는 Future Work)
...
| RQ2 | 수요 강도 K = 50 (baseline 18% 주문률) → K = 100 (stress 36%) 로 증가하면 dominant mode 가 전환되는가? | K별 dominant mode (baseline vs stress) | 모드간 KPI 격차 | K-stratified 비교 + bootstrap CI |
...
|------------|--------------|----------|
| **K=50 시나리오 1건** | **5F × 500 m² 빌딩 (280인) 의 점심피크 1h** | 50/280 ≈ 18% 주문률 — 한국 사무직 점심 배달 문화와 정합 |
| **K=100 시나리오 1건** | 동일 빌딩의 수요 stress 조건 1h | 100/280 ≈ 36% 주문률 — 금요일·악천후 surge 등 high-demand day |
| K ∈ {50, 100} (1차 실험) | 본 paper 의 baseline + stress 2-condition 대조 | E1~E6 모든 실험은 K=50 baseline, K=100 stress 두 강도에서 비교 |
| K ∈ {200, 300} (Future) | 동일 빌딩의 saturated demand 또는 더 큰 빌딩 | 5F-K=50/100 결과 확인 후 결정 |
| K ∈ {500, 1000} (Tier-2) | 더 큰 빌딩 또는 longer-horizon | stratum 내 horizon 혼재 (1h/2h/4h) 처리 필요 |
...
│  (5F × 500 m² networkx + 로비 zone + LockerBank;               │
│   10F 확장은 1차 실험 후 결정)                                  │
└──────────────────────┬──────────────────────────────────────┘
                       v
┌─────────────────────────────────────────────────────────────┐
...
- **K-stratum 추정 (1차 실험 대상)**: K ∈ **{50, 100}** 별 λ_K(t) 분리 산출. 모두 horizon ≈ 1h 로 일관 → 5F 빌딩 점심피크 윈도우와 직접 정합.
- **K-stratum 추정 (Future, K=200/300 fitted 보유)**: STAGE 1 분석 단계에서 K ∈ {200, 300} 의 λ_K(t) 도 적합·저장되어 있으나 (`fitted_params.json` 의 `by_K`), 1차 실험에선 사용하지 않음.
- **K-stratum 추정 (Tier-2)**: K ∈ {500, 1000} 은 stratum 내 horizon 혼재로 (a) 60min truncate 또는 (b) horizon sub-stratify 처리 후 별도 트랙으로 분석.
- **K-stratum 부트스트랩 CI**: K=50 시나리오 표본이 2건으로 적은 점을 보완하기 위해 STAGE 1 fit 단계에서 K=50 + K=100 + K=200 + K=300 보조 표본을 결합, 시나리오 단위 부트스트랩 (B=1000 resampling) 으로 시점별 95% CI 산출. 1차 실험 입력으로는 K=50 / K=100 의 λ_K 만 사용.
...
- **K=50 stratum 해석**: 시나리오당 약 50건이 1시간 윈도우에 분포 → 시나리오당 평균 50 ord/h, 피크 약 100~150 ord/h. 5F × 500 m² (280인 점유) 빌딩의 점심피크 18% 주문률과 직접 부합.
- **K=100 stratum 해석 (stress)**: 시나리오당 100건/1h, 36% 주문률. 동일 빌딩의 high-demand day 시뮬레이션.
...
**1차 실험 baseline 은 5F 단일 typology** 로 한국 표준 소형 사무 빌딩 (500 m²/층, footprint 22m × 23m, 약 280인 점유) 을 모델링한다. 10F 확장 (E3 typology) 은 1차 실험 결과 분석 후 별도 진행한다 (Future Work).

```
Baseline 5F (Korean 소형 사무, ~280인)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 5F  [사무실 7개]──복도(20m)──[사무실]    (~56인)
 4F  [사무실 7개]──복도(20m)──[사무실]    (~56인)
 3F  [사무실 7개]──복도(20m)──[사무실]    (~56인)
 2F  [사무실 7개]──복도(20m)──[사무실]    (~56인)
 1F  [로비 + 핸드오프 zone + 로봇 상시 대기]   (출입·접객·로봇 대기)
B1F  [충전 도크]                              (서비스 zone, SOC 임계점에서만 사용)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  EV1: 사람 전용  (수용 10명)
  EV2: 사람+로봇 공용 (로봇 1대 → 사람 6명 / 로봇 0대 → 10명)
  footprint: 22m × 23m,  층고: 3.6 m,  층당 500 m²
```

> **10F E3 (Future)**: 600 m²/층, 24m corridor, EV 3대 (2 people + 1 shared), ~667인. 1차 실험 검증 완료 후 진행.
...
| 파라미터 | Baseline (5F Korean 소형) | 비고 / 근거 |
|---------|-------|------|
| 층 구조 | B1F ~ 5F (총 6 level) | |
| **층당 면적** | **500 m² (≈ 150평)** | 한국 중소형 PMO 범위 330~660 m² 중간 |
| **Footprint** | **22m × 23m (거의 정사각)** | 500 m² 자연 형상 |
| **층고** | **3.6 m** | 한국 사무 표준 |
| **층당 복도 길이** | **20 m (편도, 중앙 single-loaded)** | 23m footprint 내 중앙 통로 |
| **층당 복도 폭** | **2 m** | 한국 사무 표준 |
| 사무실 수/층 | **7개** | 500 m² / 약 70 m²/실 — 20m 복도에 3m 간격 (positions 1, 4, 7, 10, 13, 16, 19) uniform |
| **사람 전용 EV** | **1대 (EV1), 수용 10명** | 5F 자율설치, 통상 1~2대 |
| **공용 EV** | **1대 (EV2); 수용 = (로봇 1대 → 사람 6명 / 로봇 0대 → 10명)** | 핸드오프 실험 필수 |
| **총 EV** | **2대** | 5F (< 6F, < 2,000 m² 의무 X) — 자율 |
| EV 물리 모델 | 가속도 1.0 m/s², 최고 속도 2.5 m/s, 도어 개폐 4초 | EV 대기시간 *내생화* |
| 충전 도크 | B1F (1대) | 로봇이 SOC<θ_charge 일 때만 호출 (STAGE2_plan.md §17) |
| 로봇 상시 대기 위치 | 1F `lobby_robot_pickup_zone` | 픽업+idle 겸용 — 매 주문마다 B1F 왕복하지 않음 |
| **층별 상주 인원** | **~56명/층 (5F 합 ≈ 280)** | 1인당 9 m² (한국 사무 표준) |
| **점심피크 주문률** | K=50: 18% / K=100: 36% (stress) | 280인 빌딩 기준 |
...
| `locker_bank` | 자동 도킹 locker | M compartments | H3 전용; **M ∈ {2, 4, 8} sweep** (소형 빌딩 적정 범위) |
| `direct_corridor` | 라이더 직접 EV 접근 | - | H0 전용; EV1·EV2 모두 가능 |
| `robot_pickup_zone` | 로봇 인계 대기 zone | 2대 | H1·H2·H3 공통; locker 옆 위치 (소형 빌딩 로봇 fleet 2~3대 가정) |

### 5.5 LockerAgent 파라미터

| 파라미터 | baseline | sweep | 출처 / 근거 |
|---------|---------|-------|------|
| compartment 수 M | 4 | {2, 4, 8} | 소형 빌딩 적정; framework §7.2 E4 sweep |
...
- 각 보행자는 EV1 (사람 전용) 우선 선택, 혼잡 시 EV2 (공용) 도 사용. 공용 EV2 에 로봇이 탑승해 있을 때는 혼잡 회피 행동 α (다음 EV 대기 확률) 반영, H1·H2 에서 라이더가 로비에 머무는 경우 α_lobby_extra = 0.10 추가

### 5.7 이동 속도

| 주체 | 복도 속도 | EV 이용 | 비고 |
|------|----------|---------|------|
| 로봇 | 1.0 m/s | EV2 (공용) 만 가능 | 혼잡 밀도 > θ 시 감속 |
| 외부 라이더 | 1.5 m/s | EV1·EV2 모두 가능 (H0 모드만) | |
| 일반 보행자 | 1.2 m/s | EV1 우선, EV2 보조 | EV 혼잡 유발 |
...
ExternalRider arrives at lobby_entry
  ...
  → selects EV (EV1 or EV2, min wait heuristic)
...
ExternalRider arrives at lobby_entry
  ...
  → Rider exits ; Robot proceeds: EV2 (shared) → target floor → office → customer drop (30 s)
...
- **행동**:
  ...
  4. EV2 (공용) 호출 → 대기 (사람·로봇 공유자원 경합) → 탑승 → 목적층
...
- **행동**:
  1. FCFS 또는 SCAN 알고리즘
  2. 물리 모델(가감속) 기반 층 이동
  3. EV2 (공용): 로봇 탑승 시 잔여 수용 감소 (10명 → 6명) + 사회적 수용성 α (사람이 다음 EV 대기 확률)
...
7. **Travel time decomposition**: 주문당 배달시간을 5성분으로 분해:
   ...
   - `EV_wait` = 로봇이 EV2 (공용) 호출 후 탑승까지 대기시간
...
- K=100: 9 × 30 = 270
- (Future) K=200: 9 × 30 = 270
- (Future) K=500: 5 × 30 = 150
- (Future) K=1000: 5 × 30 = 150
시나리오 수가 적은 stratum (K=50) 은 floor 배정 시드 다양화로 effective replication 확보.

### 7.2 실험 시나리오 (factorial design)

**1차 실험 factorial**: **4 modes × 2 K × 5F single typology × 30 seeds**
- K=50: 4 modes × 2 시나리오 × 30 seed = **240 runs**
- K=100: 4 modes × 9 시나리오 × 30 seed = **1,080 runs**
- **합계 1,320 runs** (E1 단독 = 240, E2 = 1,320 통합)
...
| **E2: K-sweep (baseline vs stress)** | 5F, baseline | H × K ∈ **{50, 100}** | Replay K=50 (2 시나리오) + K=100 (9 시나리오) × 30 seed | baseline (18%) → stress (36%) 모드 전환 |
| **E3: Typology** | (Future Work) | H × typology {5F vs 10F} | 1차 실험 후 결정 | 빌딩 규모/수직 병목 영향 |
| **E4: Locker sizing** | H=H3, **K=50, 5F** | M ∈ {2, 4, 8}, V_max ∈ {50, 70, 100} | Replay K=50 + 보강 K=100 | H3 의 locker 자원 최적 sizing |
| **E5: Rider wage sensitivity** | H ∈ {H1,H2,H3} | w_R ∈ {5k, 10k, 15k, 20k} KRW/h | Replay K=50,100 | break-even w\* 도출 |
| **E6: Pareto frontier** | full factorial 후처리 | H × K ∈ {50, 100} | E1–E2 통합 분석 (E3 typology 는 Future) | 3-stakeholder Pareto |
...
- **W_EV2**: 공용 EV (EV2) 평균 대기시간 — 핵심 vertical bottleneck 지표

...
- 핸드오프 방식은 **세 주체의 자원·시간 배분**을 동시에 결정한다.
  ...
  - **고객**: 핸드오프 단계의 추가 지연이 SLA 위반·포기로 직결
...
|---|---|---|---|---|
| RQ1 | 4가지 핸드오프 모드 간 종합 성능에 통계적으로 유의한 차이가 있는가? | 평균·95p 총 배달시간, 라이더 시간당 처리량, 고객 포기율 | EV 대기시간, 로봇 가동률 | 4-way ANOVA + Tukey HSD |
...
| DLV_DEADLINE | [8] | 절대시각(초) | 핸드오프 큐잉 허용폭 상한, 고객 포기 임계 근거 |
...
| 고객 SLA 임계 | DLV_DEADLINE − ORD_TIME | CustomerAgent τ_abandon 하한 |
...
#### 4.1.7 고객 SLA 임계 — 리드타임
- `DLV_DEADLINE − ORD_TIME` 로 정의
- **실증**: mean ≈ 57 min, median ≈ 56 min, q05 ≈ 38 min, q95 ≈ 120 min → CustomerAgent τ_abandon 기본값 범위 [q05, q95]
...
- `analysis/figures/` — λ(t), cook-time fit/QQ, VOL, lead-time, 거리, K scaling, **rider_arrival_lambda_K{50,100,200,500,1000}.png**, **travel_time_by_type.png**, **k50_per_scenario_arrivals.png**
- `analysis/fit_report.md` — 적합도 리포트
...
#### CustomerAgent
- **속성**: 위치 (층·호실; uniform random 배정, 시드 고정), 외부 주문 시각, 주문 부피 VOL, 포기 임계 τ_abandon (기본: 리드타임 q05 ≈ 38 min 을 하한으로 함), 가격 민감도 β_p
- **행동**:
  1. 주문은 외부에서 발생; STAGE 1 합성 시계열의 도착 이벤트와 매칭
  2. 대기 중 경과시간 w > τ_abandon 이면 확률적 포기 (renege)
  3. 만족도 score = σ(−β · T_e2e) 를 discrete event 로 기록
...
For each tick t:
  ...
  2. CustomerAgent: 주문 상태 갱신, deadline / 포기 평가
...
- **L_customer_abandon**: DLV_DEADLINE 위반율
- **S_customer**: 만족도 = σ(−β · T_e2e)

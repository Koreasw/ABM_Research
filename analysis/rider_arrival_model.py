...
def delivery_cost_krw(
    rider: Rider, dist_m: float, throughput_per_rider_h: float = 50.0
) -> float:
    """Provider cost of one delivery by this rider type at shop->drop dist_m.

    Cost_t(D) = (w_R_t / 3600) * (D / speed_t + service_t)
              = slope_t * D + intercept_t          (KRW; linear in distance)

    slope_t = w_R_t / (3600 * speed_t)  [KRW/m], intercept_t = w_R_t * service_t
    / 3600 [KRW]. See etc/rider_type_assignment_inventory.md §1.
    """
    w_per_sec = compute_w_R_krw_per_h(rider, throughput_per_rider_h) / 3600.0
    return w_per_sec * (float(dist_m) / rider.speed_mps + rider.service_time_sec)


def type_priority(
    riders: list[Rider], dist_m: float, throughput_per_rider_h: float = 50.0
) -> list[str]:
    """Rider types sorted by ascending delivery cost at distance dist_m.

    Deterministic (no RNG): ties broken by type name. With the fixed data1
    parameters this yields exactly three distance regimes
    (etc/rider_type_assignment_inventory.md §3):
      D < ~53 m   -> [WALK, BIKE, CAR]
      53..400 m   -> [BIKE, WALK, CAR]
      D >= ~400 m -> [BIKE, CAR, WALK]
    but the ordering is computed from slope/intercept, not hard-coded, so it
    stays correct if RIDERS parameters ever change.
    """
    return [
        r.type
        for r in sorted(
            riders,
            key=lambda r: (delivery_cost_krw(r, dist_m, throughput_per_rider_h), r.type),
        )
    ]
